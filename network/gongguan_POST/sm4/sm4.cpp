...
#define SWAP(sk,i) { unsigned int t = sk; sk = i; i = t; t = 0; }   //把数组顺序倒置
//void SWAP(unsigned int sk[32],int i) { unsigned int t = sk[i]; sk[i] = sk[31-i]; sk[31-i] = t; t = 0; }
...
static unsigned char sm4Sbox(unsigned char inch)
{
    ...
    return retVal;
//    int i = inch & 0xFF;
//    unsigned char retVal = SboxTable[i];
//    return retVal;
}
...
unsigned char* padding(unsigned char input[1000],int mode)
{
   if (mode == SM4_ENCRYPT)
   {
       int p = 16 - (strlen((char*)input) %16);
       unsigned char ret[strlen((char*)input)+p];
       memcpy(ret,input,strlen((char*)input));
       for (int i = 0; i < p; i++)
       {
          ret[strlen((char*)input) + i]  = p;
       }
       return ret;
   }
   else
   {
      int p =  input[strlen((char*)input) - 1];
      unsigned char ret[strlen((char*)input)-p];
      memcpy(ret,input,(strlen((char*)input)-p));
      return ret;
   }
...
void sm4_setkey_dec( sm4_context *ctx, unsigned char key[16] )
{
    ...
    for( i = 0; i < 16; i ++ )
    {
        SWAP( ctx->sk[ i ], ctx->sk[ 31-i] );//SWAP( ctx->sk[ i ], i);
    }
}
...
//            for( i = 0; i < 16; i++ )
//            {
//               output_temp[i] = (unsigned char)( input[i] ^ pbIV[i] );
//            }
////            printf("%d \n",strlen((char*)output));

//            sm4_one_round( ctx->sk, output_temp, output_temp );
//            memcpy( pbIV, output_temp, 16 );
//            for(int j=0;j<16;j++)
//            {
//               output[output_index] = output_temp[j];
//               printf("%02x ",output[output_index]);fflush(stdout);
//               output_index++;
//            }
