    ...
    : QWidget(parent, Qt::Tool| Qt::WindowStaysOnTopHint | Qt::FramelessWindowHint),m_lastFocusedWidget_(0)
{
    ...
    m_currentPos_ = 0;
    m_currentPage_ = 0;
	ui.btn_caplock_->setStyleSheet(QString("#btn_caplock_{color: rgb(0,0,0);background-color:rgb(255,255,255);border-radius: 7px;}#btn_caplock_:pressed{background-color: rgb(255,255,255);}"));
	ui.btn_shift_->setStyleSheet(QString("#btn_shift_{color: rgb(0,0,0);background-color:rgb(255,255,255);border-radius: 7px;}#btn_shift_:pressed{background-color: rgb(255,255,255);}"));
    ...
    for(int i = 0;i < 12;++i)
		m_btns_[46 + i]->setStyleSheet(QString("#word_") + QString::number(i) + "_{color: rgb(0,0,0);background-color: rgb(255,255,255);border-radius: 7px;}#word_" + QString::number(i) + "_:pressed{background-color: rgb(255,255,255);}");
    for(int i = 0;i < 65;++i)
    {
...
void SoftKeyBoard::buttonClicked(QWidget *w)
{
    ...
    if(!m_bIsChinese_)                //英文
    {
        if(GetBtnText(w,text,0,26) && text.size() > 0)//英文 字母
            ...
            key = Qt::Key_Space;
		//qDebug()<<__PRETTY_FUNCTION__<<text<<key; //注释
...
{
	QPainter p(this);
	p.fillRect(rect(), QColor(40,100,190,150));
}
...
void SoftKeyBoard::on_btn_language__clicked()
{
	...
	ui.widget_keyboard_->move(30,m_bIsChinese_ ? 88 : 5);
	resize(780,m_bIsChinese_ ? 355 :272);
	this->move(92,m_bIsChinese_ ? 407 : 490);
}

void SoftKeyBoard::on_btn_caplock__clicked()
{
    m_bIsUppercase_ = !m_bIsUppercase_;
	ui.btn_caplock_->setStyleSheet(QString("#btn_caplock_{color: rgb(0,0,0);background-color:") + (m_bIsUppercase_ ? "rgb(194,194,194)" : "rgb(255,255,255)") + ";border-radius: 7px;}#btn_caplock_:pressed{background-color: rgb(255,255,255);}");
    for(int i = 0;i < 26;++i)
...
void SoftKeyBoard::on_btn_shift__clicked()
{
	...
	ui.btn_shift_->setStyleSheet(QString("#btn_shift_{color: rgb(0,0,0);background-color:") + (m_bShiftPressed_ ? "rgb(194,194,194)" : "rgb(255,255,255)") + ";border-radius: 7px;}#btn_shift_:pressed{background-color: rgb(255,255,255);}");
}

bool SoftKeyBoard::InitChinese()
{
    qDebug()<<QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss:zzz");
	QFile file(":/new_keyboard/FontLibrary.txt");
    int i = 0;
...
void SoftKeyBoard::ShowChinese(QString str,int page,int pos)
{
    ...
    for(int i = 0;i < 12;++i)
		m_btns_[46 + i]->setStyleSheet(QString("#word_") + QString::number(i) + "_{color: rgb(0,0,0);background-color: rgb(255,255,255);border-radius: 7px;}#word_" + QString::number(i) + "_:pressed{background-color: rgb(194,194,194);}");
    if(m_btns_[46 + pos]->text().isEmpty())
        pos = m_currentPos_;
    if(!m_btns_[46 + pos]->text().isEmpty())
		m_btns_[46 + pos]->setStyleSheet(QString("#word_") + QString::number(pos) + "_{color: rgb(0,0,0);background-color: rgb(194,194,194);border-radius: 7px;}#word_" + QString::number(pos) + "_:pressed{background-color: rgb(194,194,194);}");
    m_strPingying_ = str;
    m_currentPage_ = page;
    m_currentPos_ = pos;
}
void SoftKeyBoard::close_keyboard()
{
//	this->hide();
//	qDebug()<<"%%%%%%%%%%%%%%%%";
}
